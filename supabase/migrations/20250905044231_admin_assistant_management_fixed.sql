-- Location: supabase/migrations/20250905044231_admin_assistant_management_fixed.sql
...
-- FIXED: Proper type casting for user_role enum comparison
...
BEGIN
    ...
    SELECT id INTO admin_user_id FROM auth.users WHERE email = 'ian@ianmclayton.com' LIMIT 1;
    SELECT id INTO test_user_id FROM auth.users WHERE email = 'info@usmbok.com' LIMIT 1;
    ...
    VALUES 
        (admin_user_id, 'ian@ianmclayton.com', 'Ian Clayton', 'admin'::public.user_role),
        (test_user_id, 'info@usmbok.com', 'Joe Smith', 'member'::public.user_role)
